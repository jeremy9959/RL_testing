...
by selling the most amount of stock, and purchasing the smallest amount of inventory.
...
Here we compute P(X = k) from 0 <= k <= n and store each probability in our list called self.distribution
...
from our self.customers list which contains integers ranging from 0 to n - 1. Each integer in this list has a
defined probability of being chosen which is defined in our distribution list; self.distribution, where
the probability of each integer hitting is stored in the same index in our distribution list.
...
had a positive or negative outcome (more on this later). Anyways, the reward is defined in the environment as:
...
$$
\text{number of customers} <= \text{orders_sold} - \text{orders_missed} <= \text{number of customers}
$$
...
is updated. Finally, $\alpha$ is responsible for how quickly our state_action lookup table is updated.
...
We take this prediction, and find update its accuracy by finding the difference between the prediction and the actual
